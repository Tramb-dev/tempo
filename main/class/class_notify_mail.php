 ...
 */
    ...
    */
    ...
    private function MailSend($header, $body) {
		...
		// Si le corps est en HTML, on remplace les sauts de ligne par des <br />
        ...
        else
		{
			...
			//echo "TO = $to<hr />SUBJECT = " . nl2br($this->EncodeHeader($this->Subject)) . "<hr />BODY = " . nl2br($body) . "<hr />HEADER = " . nl2br($header) . "<hr />";
		}
    ...
    private function SmtpConnect() {
        ...
        while($index < count($hosts) && $connection == false)
        {
            ...
            if($this->smtp->Connect($host, $port, $this->Timeout))
            {
                ...
                if($this->SMTPAuth)
                {
                    if(!$this->smtp->Authenticate(SMTP_USERNAME, SMTP_PASSWORD))
                    {
	...
	public function set_file($filename)
	{
		...
		{
			$this->SetError("template" . $filename);
		}

		$updated = dirname($filename) . '/' . get_file_data($filename, 'filename') . '.updated';
		$this->Body = (file_exists($updated)) ? file_get_contents($updated) : file_get_contents($filename);
	}
	...
	public function set_vars($vars = array())
	{
		...
		foreach ($vars AS $key => $value)
		{
			if (is_array($value))
			{
				$this->Body = str_replace('{' . $key . '}', $value[0], $this->Body);
			}
			else
			{
				$this->Body = str_replace('{' . $key . '}', $value, $this->Body);
